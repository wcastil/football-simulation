...
#RM = data_ops.RunData('2013_nfl_pbp_through_wk_12.csv')
#Xa, Yalltrain = RM.RunTrainData()
RM = data_ops.PassData('2013_nfl_pbp_through_wk_12.csv')
Pall = RM.PassTrainData(teamname='SD')
Yalltrain = []
Xa =[]
for i in range(len(Pall)):
  if Pall[i][-1] != 0:
      Xa.append( Pall[i][0:-1] )
      Yalltrain.append(Pall[i][-1])
